...
这里是新建`parallel`和`parallel_func`两个attribute，用于后续并发粒度控制
...
这里新建了`to_once_call_arg1`、`to_once_call_arg2`、` rayon_join`三个lang_item，用于让编译器可以调用后端并发框架(rayon)接口



## 2. 修改borrow check过程并输出mir及nll_facts编译中间信息

编译器通过判断用户是否加入了`parallelization`feature, 选择是否在执行borrow check时将生成的中间结果（mir及nll_facts）输出出来，以便后续分析。
...
这里的更改是表明在开启`parallelization`feature时，告知编译器输出函数nll_facts信息。
...
这里的更改时表明在开启`parallelization`feature时，告知编译器输出函数mir信息。
...
这里新建了dump_mir_unchecked函数，让编译器输出函数mir信息到本地文件。



## 3. 从函数MIR中识别无数据依赖的函数调用

下面这部分代码由chunmiao实现，用于调用crepe数据分析引擎分析之前过程中输出的nll_facts及mir信息。
...
接下来几个函数为chunmiao实现，用于识别mir(参数body)中无数据依赖的函数并输出为哈希表
...
## 4.修改编译器MIR优化过程，添加Parallelism转换
...
这里的更改时表明在开启`parallelization`feature时，告知编译器进行Parallelism转换



compiler/rustc_mir_transform/src/parallelization.rs

此run_pass函数为mir转换详细过程，输入参数tcx用于访问编译器中间数据，body为要转换的mir。
...
## 4. 解析nll_facts及mir的python脚本

见temp/extract_input.py，由chunmiao实现



## 5. 工具crate包parallelization_builtins

见temp/parallelization_builtins，加入项目依赖中，用于告知编译器如何调用rayon接口